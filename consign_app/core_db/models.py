...
class Investor(StampMixin):
    ...
    document = models.CharField(max_length=32, db_index=True)           # CPF/CNPJ
    email = models.EmailField(max_length=255, blank=True)
    phone_number = models.CharField(max_length=32, blank=True)
    kyc_status = models.CharField(max_length=32, blank=True)            # pending|approved|rejected
    suitability_profile = models.CharField(max_length=64, blank=True)
    preferred_payout_method = models.CharField(max_length=16, blank=True)  # pix|ted|wallet
...
class Wallet(StampMixin):
    ...
    owner_type = models.CharField(max_length=32)                         # investor|platform|escrow
    owner_id = models.UUIDField()                                        # referência lógica
...
class LoanOffer(StampMixin):
    ...
    status = models.CharField(max_length=32, default="draft")            # draft|open|funded|closed
...
class Contract(StampMixin):
    ...
    creditor_type = models.CharField(max_length=32, default="investor")  # investor|vehicle
    creditor_id = models.UUIDField(null=True, blank=True)                # ref. lógica
    ...
    requester_identifier_key = models.CharField(max_length=255, blank=True, db_index=True)
...
class Installment(StampMixin):
    ...
    installment_key = models.CharField(max_length=255, blank=True, db_index=True)

class Payment(StampMixin):
    payment_id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    installment = models.ForeignKey(Installment, on_delete=models.PROTECT)
    contract = models.ForeignKey(Contract, on_delete=models.PROTECT)
    source = models.CharField(max_length=32)                              # convenio|pix|boleto|ted
    amount = models.DecimalField(**MONEY)
    paid_at = models.DateTimeField()
    status = models.CharField(max_length=32, default="processing")        # processing|settled|failed
    external_reference = models.CharField(max_length=255, blank=True, db_index=True)
    reconciliation_batch = models.ForeignKey(
        "Reconciliation", null=True, blank=True, on_delete=models.SET_NULL
    )
    raw_payload = models.JSONField(default=dict, blank=True)
    pix_transfer_key = models.CharField(max_length=255, blank=True, db_index=True)
    end_to_end_id = models.CharField(max_length=255, blank=True, db_index=True)
    transaction_key = models.CharField(max_length=255, blank=True, db_index=True)
    request_control_key = models.CharField(max_length=255, blank=True, db_index=True)
    idempotency_key = models.CharField(max_length=255, blank=True, db_index=True)

class Disbursement(StampMixin):
    disbursement_id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    contract = models.ForeignKey(Contract, on_delete=models.PROTECT)
    method = models.CharField(max_length=16)                               # pix|ted
    amount = models.DecimalField(**MONEY)
...
class KycRisk(StampMixin):
    ...
    subject_type = models.CharField(max_length=16)                         # investor|borrower
    ...
    natural_person_key = models.CharField(max_length=255, blank=True, db_index=True)
    legal_person_key   = models.CharField(max_length=255, blank=True, db_index=True)
    external_reference = models.CharField(max_length=255, blank=True, db_index=True)

class ConsignmentAgreement(StampMixin):
    consignment_agreement_id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    borrower = models.ForeignKey(
    "Borrower",
    on_delete=models.CASCADE,
    related_name="agreements",
    )
    issuer = models.CharField(max_length=64)                               # órgão/empresa
...
class Document(StampMixin):
    ...
    owner_type = models.CharField(max_length=32)                           # investor|borrower|contract|kyc
...
class WebhookEvent(StampMixin):
    ...
    direction = models.CharField(max_length=16)                            # inbound|outbound
...
class Reconciliation(StampMixin):
    ...
    source = models.CharField(max_length=32, default="bank")               # bank|qitech|platform
...
class LedgerEntry(StampMixin):
    ledger_entry_id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
